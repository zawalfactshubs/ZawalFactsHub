...
const tipText = document.getElementById('weight-tip-text');
const generateBtn = document.getElementById('generate-weight-tip');
const grid = document.getElementById('weight-tips-grid');
...
function displayTips(list, page = 1) {
  ...
  pageTips.forEach((t, index) => {
    ...
    card.className = 'card health-card weight-card';
    
    card.innerHTML = `
      <div class="card-content">
        <h3>Tip ${globalIndex + 1}</h3>

        <p class="weight-text health-text health-short">
          ${t.short}
        </p>

        <p class="weight-text health-text">
          ${t.long}
        </p>
...
function listenCard(btn) {
  ...
  const textElements = card.querySelectorAll('.weight-text');
  let fullText = '';
...
function shareCard(btn) {
  ...
  html2canvas(card, { scale: 6, useCORS: true }).then(canvas => {
    canvas.toBlob(blob => {
      const file = new File([blob], 'weight-tip.png', { type: 'image/png' });
      
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        navigator.share({
          files: [file],
          title: 'Weight Management Tip',
          text: 'Healthy weight tips from Zawal Facts Hub!'
        });
      }
...
function searchWeightTips() {
  ...
  if (filtered.length) {
    ...
    document.getElementById('weight-no-results').style.display = 'none';
  } else {
    grid.innerHTML = '';
    document.getElementById('weight-no-results').style.display = 'block';
  }
}

// ----------------- EVENTS -----------------
searchInput.addEventListener('input', searchWeightTips);
searchInput.addEventListener('keydown', e => { if (e.key === 'Enter') searchWeightTips(); });
