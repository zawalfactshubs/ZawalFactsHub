...
const generateBtn = document.getElementById('generate-mental-tip');
const grid = document.getElementById('mental-tips-grid');
const searchInput = document.getElementById('mental-search-input');
...
function displayTips(list, page = 1) {
  ...
  pageTips.forEach((t, index) => {
    ...
    card.className = 'card health-card mental-card';
    
    card.innerHTML = `
      <div class="card-content">
        <h3>Tip ${globalIndex + 1}</h3>

        <p class="mental-text health-text health-short">
          ${t.short}
        </p>

        <p class="mental-text health-text">
          ${t.long}
        </p>
...
function listenCard(btn) {
  const card = btn.closest('.card.mental-card');
  const textElements = card.querySelectorAll('.mental-text');
  let fullText = '';
...
function downloadTip(btn) {
  ...
  setTimeout(() => {
    html2canvas(card, { scale: 6, useCORS: true }).then(canvas => {
      ...
      link.download = 'mental-health-tip.png';
...
function shareCard(btn) {
  ...
  html2canvas(card, { scale: 6, useCORS: true }).then(canvas => {
    canvas.toBlob(blob => {
      const file = new File([blob], 'mental-health-tip.png', { type: 'image/png' });
      
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        navigator.share({
          files: [file],
          title: 'Mental Health Tip',
          text: 'Check out this mental health tip from Zawal Facts Hub!'
        });
      }
...
function searchMentalTips() {
  ...
  if (filtered.length) {
    ...
    document.getElementById('mental-no-results').style.display = 'none';
  } else {
    grid.innerHTML = '';
    document.getElementById('mental-no-results').style.display = 'block';
  }
}

// ----------------- EVENTS -----------------
searchInput.addEventListener('input', searchMentalTips);
searchInput.addEventListener('keydown', e => { if (e.key === 'Enter') searchMentalTips(); });
