...
const generateBtn = document.getElementById('generate-hydration-tip');
const grid = document.getElementById('hydration-tips-grid');
const searchInput = document.getElementById('hydration-search-input');

let tipsPerPage = 15;
let currentPage = 1;

// ----------------- LOAD JSON -----------------
fetch('data/hydration.json')
  .then(res => res.json())
  .then(data => {
    tips = data;
    showRandomTip();
...
function displayTips(list, page = 1) {
  ...
  pageTips.forEach((t, index) => {
    ...
    card.className = 'card health-card hydration-card';
    
    card.innerHTML = `
      <div class="card-content">
        <h3>Tip ${globalIndex + 1}</h3>

        <p class="hydration-text health-text health-short">
          ${t.short}
        </p>

        <p class="hydration-text health-text">
          ${t.long}
        </p>
...
function listenCard(btn) {
  ...
  const textElements = card.querySelectorAll('.hydration-text');
  let fullText = '';
...
function downloadTip(btn) {
  ...
  setTimeout(() => {
    html2canvas(card, { scale: 6, useCORS: true }).then(canvas => {
      ...
      link.download = 'hydration-tip.png';
...
function shareCard(btn) {
  ...
  html2canvas(card, { scale: 6, useCORS: true }).then(canvas => {
    canvas.toBlob(blob => {
      const file = new File([blob], 'hydration-tip.png', { type: 'image/png' });
      
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        navigator.share({
          files: [file],
          title: 'Hydration Tip',
          text: 'Stay hydrated with this tip from Zawal Facts Hub!'
        });
      }
...
function searchHydrationTips() {
  ...
  if (filtered.length) {
    ...
    document.getElementById('hydration-no-results').style.display = 'none';
  } else {
    grid.innerHTML = '';
    document.getElementById('hydration-no-results').style.display = 'block';
  }
}

// ----------------- EVENTS -----------------
searchInput.addEventListener('input', searchHydrationTips);
searchInput.addEventListener('keydown', e => { if (e.key === 'Enter') searchHydrationTips(); });
