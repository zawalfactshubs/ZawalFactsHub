...
const tipText = document.getElementById('fitness-tip-text');
const generateBtn = document.getElementById('generate-fitness-tip');
const grid = document.getElementById('fitness-tips-grid');
const searchInput = document.getElementById('fitness-search-input');
...
function displayTips(list, page = 1) {
  ...
  pageTips.forEach((t, index) => {
    ...
    card.className = 'card health-card fitness-card';
    
    card.innerHTML = `
      <div class="card-content">
        <h3>Tip ${globalIndex + 1}</h3>

        <p class="fitness-text health-text health-short">
          ${t.short}
        </p>

        <p class="fitness-text health-text">
          ${t.long}
        </p>
...
function listenCard(btn) {
  const card = btn.closest('.card.fitness-card');
  const textElements = card.querySelectorAll('.fitness-text');
  let fullText = '';
...
function shareCard(btn) {
  ...
  html2canvas(card, { scale: 6, useCORS: true }).then(canvas => {
    canvas.toBlob(blob => {
      const file = new File([blob], 'fitness-tip.png', { type: 'image/png' });
      
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        navigator.share({
          files: [file],
          title: 'Fitness Tip',
          text: 'Check out this fitness tip from Zawal Facts Hub!'
        });
      }
...
function searchFitnessTips() {
  ...
  if (filtered.length) {
    ...
    document.getElementById('fitness-no-results').style.display = 'none';
  } else {
    grid.innerHTML = '';
    document.getElementById('fitness-no-results').style.display = 'block';
  }
}

// ----------------- EVENTS -----------------
searchInput.addEventListener('input', searchFitnessTips);
searchInput.addEventListener('keydown', e => { if (e.key === 'Enter') searchFitnessTips(); });
