...
const tipText = document.getElementById('habit-tip-text');
const generateBtn = document.getElementById('generate-habit-tip');
const grid = document.getElementById('habit-tips-grid');
...
function displayTips(list, page = 1) {
  ...
  pageTips.forEach((t, index) => {
    ...
    card.className = 'card health-card habit-card';
    
    card.innerHTML = `
      <div class="card-content">
        <h3>Tip ${globalIndex + 1}</h3>

        <p class="habit-text health-text health-short">
          ${t.short}
        </p>

        <p class="habit-text health-text">
          ${t.long}
        </p>
...
function listenCard(btn) {
  const card = btn.closest('.card.habit-card');
  const textElements = card.querySelectorAll('.habit-text');
  let fullText = '';
...
function shareCard(btn) {
  ...
  html2canvas(card, { scale: 6, useCORS: true }).then(canvas => {
    canvas.toBlob(blob => {
      const file = new File([blob], 'habit-tip.png', { type: 'image/png' });
      
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        navigator.share({
          files: [file],
          title: 'Healthy Habit Tip',
          text: 'Check out this healthy habit tip from Zawal Facts Hub!'
        });
      }
...
function searchHabitTips() {
  ...
  if (filtered.length) {
    ...
    document.getElementById('habit-no-results').style.display = 'none';
  } else {
    grid.innerHTML = '';
    document.getElementById('habit-no-results').style.display = 'block';
  }
}

// ----------------- EVENTS -----------------
searchInput.addEventListener('input', searchHabitTips);
searchInput.addEventListener('keydown', e => { if (e.key === 'Enter') searchHabitTips(); });
