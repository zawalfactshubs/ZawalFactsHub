...
const humanGrid = document.getElementById('human-facts-grid');
    ...
    card.innerHTML = `
      ...
      <div class="card-content" style="padding:10px;">
        ...
        <div style="display:flex;gap:5px;flex-wrap:wrap;margin-top:5px;">
          ...
          <button style="flex:1;padding:8px;background:#CD7F32;color:#F5F5F5;border:none;border-radius:8px;cursor:pointer;font-weight:700;" onclick="listenFact('${fact.fact}')">
            🗣️ Listen
          </button>
...
<button style="flex:1;padding:8px;background:#CD7F32;color:#F5F5F5;border:none;border-radius:8px;cursor:pointer;font-weight:700;"
  ...
  📤 Share
          ...
          <button style="flex:1;padding:8px;background:#CD7F32;color:#F5F5F5;border:none;border-radius:8px;cursor:pointer;font-weight:700;" onclick="downloadCard(this)">
            💾 Download
          </button>
...
function downloadCard(btn) {
  ...
  const factText = card.querySelector('p').innerText;
  ...
  image.onload = () => {
    ...
    context.fillRect(50, imgHeight + 30, canvas.width - 100, canvas.height - imgHeight - 80);
    context.globalAlpha = 1;
    
    context.fillStyle = "#CD7F32";
    context.font = "bold 36px Arial";
    context.textAlign = "center";
    wrapText(context, factText, canvas.width / 2, imgHeight + 80, canvas.width - 120, 40);
    
    context.fillStyle = "#1C2B2D";
    context.font = "bold 24px Arial";
    context.textAlign = "right";
    context.fillText("© Zawal Facts Hub", canvas.width - 60, canvas.height - 30);
    
    const link = document.createElement('a');
    link.href = canvas.toDataURL();
    link.download = 'human-fact.png';
    link.click();
  }
}
...
searchInput.addEventListener('input', () => {
  ...
  if (!query) {
    displayFacts(facts); // show all facts if input is empty
